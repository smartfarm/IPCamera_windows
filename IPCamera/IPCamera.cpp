...
//设置配置文件的路径
char sConfigFile[] = "..\\IPCameraconfig.ini";
...
int main()
{
	...
	char temp[100];
	ifstream fin(sConfigFile, ios::in);
	...
	sscanf_s(temp, "%d%d%d%d%d",&iCaptureNumber,&iCaptureCycleTime,&iRecordTime,&iRecordNum);
	...
	HANDLE *hDevice = new HANDLE[iDeviceNum];
	for (int i = 0; i < iDeviceNum; i++)
	{
		CAM t;
...
/*
DWORD WINAPI Record(PVOID param)
{
	struct CAM2
	{
		LONG lRealPlayHandle;
	...
	//判断路径是否存在
	CString sTemp;
	sTemp.Format("%s%s\\",sRecordPath, IPAddress);
	DWORD dwRet = GetFileAttributes(sTemp);
	if ((dwRet == -1) || !(dwRet & FILE_ATTRIBUTE_DIRECTORY))
	{
		CreateDirectory(sTemp, NULL);
	...
	char sRecordFilename[256];
	for (int i = 0; i < iRecordNum; i++)
	{
		time = CTime::GetCurrentTime();
		sprintf(sRecordFilename, "%s%s\\Record_%04d%02d%02d_%02d%02d%02d_%02d.mp4",
				sRecordPath,
				IPAddress,
				time.GetYear(),
				time.GetMonth(),
				...
				time.GetSecond(),
				GetTickCount());
		NET_DVR_SaveRealData(lRealPlayHandle, sRecordFilename);//开始录制
		Sleep(iRecordTime);
		NET_DVR_StopSaveRealData(lRealPlayHandle);//结束录制
	}
	return 0;
}
*/